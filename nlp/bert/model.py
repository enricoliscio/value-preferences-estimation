...
from nlp.bert.utils import format_time, classification_info

value_labels = ['kosten', 'land', 'leiding', 'samen', 'zelf']

DEFAULT_CONFIG = {
    "label_names": value_labels,
    "epochs": 3,
    ...
    "optim": AdamW,
    'max_seq_length': 64,
    "dutch": "pdelobelle/robbert-v2-dutch-base",
    "english": "cardiffnlp/twitter-roberta-base-sentiment",
    "dropout": 0.1
}
...
class MultiLabelBertBase:
    def __init__(self, config=DEFAULT_CONFIG, model=None):
        ...
        if model:
            ...
            self.model = AutoModelForSequenceClassification.from_pretrained(
                self.config['name'], num_labels=len(self.config['label_names']), ignore_mismatched_sizes=True,
                problem_type='multi_label_classification', hidden_dropout_prob=self.config['dropout'])
    ...
    def train(self, train_dataset, dev_dataset, validation=True, verbose=True):
                ...
                if batch_count % validation_step == 0 and validation:
                    ...
                    if loss < best_loss:
                        ...
                        if verbose:
                            ...
                            print(classification_info(true_labels, predictions, target_names=self.config['label_names']))
    ...
    def predict_prob(self, dataset):
        dataset.to(self.device)
        loader = DataLoader(dataset, batch_size=self.config['batch_size'])
        y_predicted, y_true, _ = self.evaluate(loader)

        # Apply sigmoid on logits to obtain probability scores
        sigmoid = torch.nn.Sigmoid()
        probabilities = sigmoid(y_predicted)

        return probabilities.cpu().numpy(), y_true.cpu().numpy()
